...
const compareString = (leftString, rightString) => {
    if (!isString(leftString) || !isString(rightString)) return false
    return leftString.toUpperCase() === rightString.toUpperCase()
}

const compareStringInArray = (itemString, arrayItems) => {
    if (!itemString || !Array.isArray(arrayItems)) return false
    let findItem = arrayItems.filter((item) => compareString(item, itemString))
